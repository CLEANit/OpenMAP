...
class Poscar:
    def __init__(self, filename="POSCAR", use_direct=True):
        ...
        if len(potcar_lines) == 0:
            print
            "Cannot find atomic information in POTCAR. Missing POTCAR?"
            exit(1)
        ...
        if volumescaling:
            ...
            unscaledvolume = self.a[0] * self.b[1] * self.c[2] - self.a[0] * self.b[2] * self.c[1] + self.a[1] * self.b[
                2] * self.c[0] - self.a[1] * self.b[0] * self.c[2] + self.a[2] * self.b[0] * self.c[1] - self.a[2] * \
                             self.b[1] * self.c[0]
        ...
        for i in range(offset, offset + n_atoms):
            if poscardata[i] != "" and not ''.join(poscardata[i].split()).isalpha():
                    ...
                    if use_direct:
                        if direct_in_file:
                            ...
                            if alpha_angle == 90.0 and beta_angle == 90.0 and gamma_angle == 90.0:
                                ...
                                print
                                "Cannot yet initialize from Cartesian coordinates in non-orthorhombic cells"
                                exit(1)
                    else:
                        if direct_in_file:
                ...
                print
                "Reached end of file when reading POSCAR (read %d lines)" % i
                exit(1)
    ...
    def volume(self):
        ...
        return self.a[0] * self.b[1] * self.c[2] - self.a[0] * self.b[2] * self.c[1] + self.a[1] * self.b[2] * self.c[
            0] - self.a[1] * self.b[0] * self.c[2] + self.a[2] * self.b[0] * self.c[1] - self.a[2] * self.b[1] * self.c[
                   0]
    ...
    def reciprocal_a(self):
        return [cross(self.b, self.c)[0] / abs(dot(self.a, cross(self.b, self.c))), cross(self.b, self.c)[1] /
                abs(dot(self.a, cross(self.b, self.c))),
                cross(self.b, self.c)[2] / abs(dot(self.a, cross(self.b, self.c)))]

    def reciprocal_b(self):
        return [cross(self.c, self.a)[0] / abs(dot(self.b, cross(self.c, self.a))), cross(self.c, self.a)[1] /
                abs(dot(self.b, cross(self.c, self.a))),
                cross(self.c, self.a)[2] / abs(dot(self.b, cross(self.c, self.a)))]

    def reciprocal_c(self):
        return [cross(self.a, self.b)[0] / abs(dot(self.c, cross(self.a, self.b))), cross(self.a, self.b)[1] /
                abs(dot(self.c, cross(self.a, self.b))),
                cross(self.a, self.b)[2] / abs(dot(self.c, cross(self.a, self.b)))]
...
if "ENCUT" not in incar:
    ...
    if cutoff < enmax:
        logging.info(
            "ENCUT is lower then the recommended ENMAX in the POTCAR file. This could lead to artificially compression if you relax the cell.")
    if "ISIF" in incar:
...
if "IBRION" in incar and incar["IBRION"] == "1" and "POTIM" not in incar:
    logging.warning(
        "POTIM is not set, even though IBRION=1 is selected. VASP's default value of 0.5 may not be optimal.")
...
if "ISPIN" in incar and int(incar["ISPIN"]) == 2 and "MAGMOM" not in incar and "ICHARG" in incar and int(
        incar["ICHARG"]) != 1:
...
if "LDAUU" in incar and atom_kinds_given(incar["LDAUU"]) != len(cell.atom_counts()):
    logging.error("LDAUU and number of atoms is inconsistent. %d != %d" % (
    atom_kinds_given(incar["LDAUU"]), len(cell.atom_counts())))

if "LDAUJ" in incar and atom_kinds_given(incar["LDAUJ"]) != len(cell.atom_counts()):
    logging.error("LDAUJ and number of atoms is inconsistent.%d != %d" % (
    atom_kinds_given(incar["LDAUJ"]), len(cell.atom_counts())))
...
if options.cores:
    ...
    if options.nodes:
        ...
        if nodes > 16 and cores == nodes * 16 and "LHFCALC" not in incar:
            logging.info(
                "You might see better performance if you drop to 8-12cores/node using #SBATCH --ntasks-per-node 8/12")
