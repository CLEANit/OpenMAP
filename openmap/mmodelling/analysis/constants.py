...
try:
    au_in_electricpolarizability = codata.value(
        'atomic unit of electric polarizability')  # C2 m2 /J
except KeyError:  # Mistake in the name in the previous version of scipy
    au_in_electricpolarizability = codata.value(
        'atomic unit of electric polarizablity')  # C2 m2 /J

try:
    au_in_firsthyperpolarizability = codata.value(
