...
class CategoryWriter(object):
    ...
    def generate_descriptors(self, df, id_colm='material_id', features=None):
        ...
        filepath = os.path.join(loc, '{}.json'.format(self.project_name))
        with open(filepath, 'w') as fp:
            json.dump(descriptors, fp, indent=6)
        logger.info(' File "{}.json" was created'.format(self.project_name))
    ...
    def write_categories(self, with_descriptors=True):
        ...
        for param_name in self.param_names:
            ...
            dir_name = Path(os.path.join(loc, '../CatDetails'))
            # dir_name.mkdir(parents=True, exist_ok=True)
            try:
                dir_name.mkdir(parents=True, exist_ok=False)
            except FileExistsError:
                logger.warning('Folder "CatDetails" is already there')
            else:
            ...
            logger.info(
                f'File cat_details_{param_name}.pkl was created ')
