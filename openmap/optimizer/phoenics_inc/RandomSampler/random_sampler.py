...
import numpy as np
from utilities import Logger, PhoenicsUnknownSettingsError
...
class RandomSampler(Logger):
    def __init__(self, config_general, config_params):
        ...
        if self.config_general.sampler == 'sobol':
            ...
            from RandomSampler.uniform import UniformCategorical, UniformDiscrete
            ...
            from RandomSampler.uniform import (
                UniformCategorical,
                UniformContinuous,
                UniformDiscrete,
            )
            ...
            PhoenicsUnknownSettingsError(
                'did not understanding sampler setting: "%s".\n\tChoose from "uniform" or "sobol"'
                % self.config_general.sampler
            )
    ...
    def draw(self, num=1):
        ...
        for param_index, param_settings in enumerate(self.config_params):
            ...
            if param_settings['type'] == 'continuous':
                sampled_values = self.continuous_sampler.draw(
                ...
                PhoenicsUnknownSettingsError(
                    'did not understand parameter type: "%s" of parameter "%s".\n\t(%s) Choose from "continuous", "discrete" or "categorical"'
                    % (param_settings['type'], self.config_params[param_index]['name'], self.template)
                )
    ...
    def perturb(self, pos, num=1, scale=0.05):
        ...
        for param_index, param_settings in enumerate(self.config_params):
            ...
            if param_settings['type'] == 'continuous':
                sampled_values = self.continuous_sampler.draw(
                    -scale, scale, (num, param_settings['size']))
                sampled_values *= specs['high'] - specs['low']
                close_samples = pos[param_index] + sampled_values
                close_samples = np.where(
                    close_samples < specs['low'], specs['low'], close_samples)
                close_samples = np.where(
                    close_samples > specs['high'], specs['high'], close_samples)
                samples.append(close_samples)
            elif param_settings['type'] in ['categorical', 'discrete']:
                sampled_values = pos[param_index] * \
                    np.ones((num, param_settings['size'])).astype(np.float32)
                samples.append(sampled_values)
            else:
                PhoenicsUnknownSettingsError('did not understand settings')
        samples = np.concatenate(samples, axis=1)
        return samples
    ...
    def normal_samples(self, loc=0.0, scale=1.0, num=1):
        ...
        for param_index, param_settings in enumerate(self.config_params):
            ...
            if param_settings['type'] == 'continuous':
                ...
                sampled_values = (
                    np.random.normal(
                        0.0, scale * param_range, (num, param_settings['size'])) + loc[param_index]
                )
                samples.append(sampled_values)
            elif param_settings['type'] == 'categorical':
                sampled_values = self.categorical_sampler.draw(
                ...
                PhoenicsUnknownSettingsError(
                    ...
                    % (param_settings['type'], param_settings['name'], self.template)
                )
        samples = np.concatenate(samples, axis=1)
        return samples
