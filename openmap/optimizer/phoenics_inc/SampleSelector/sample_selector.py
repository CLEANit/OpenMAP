...
class SampleSelector(Logger):
    def __init__(self, config):
        self.config = config
        Logger.__init__(self, 'SampleSelector',
    ...
    def compute_exp_objs(self, proposals, kernel_contribution, batch_index, return_index, result_dict=None):
        ...
        for sample_index, sample in enumerate(samples):
            ...
            kernel_contrib = (
                num + self.sampling_param_values[batch_index]) * inv_den
    ...
    def select(self, num_samples, proposals, kernel_contribution, sampling_param_values, obs_params):
        ...
        if self.config.get('parallel'):
            ...
            for batch_index in range(len(sampling_param_values)):
                for split_index in range(num_splits):
                    ...
                    process = Process(
                        ...
                        args=(
                            proposals[:, split_start:split_end],
                            kernel_contribution,
                            batch_index,
                            return_index,
                            result_dict,
            ...
            for batch_index in range(len(sampling_param_values)):
                ...
                result_dict[return_index] = self.compute_exp_objs(
                    proposals, kernel_contribution, batch_index, return_index
                )
        ...
        for batch_index in range(len(sampling_param_values)):
            ...
            distances = [np.sum((obs_params - batch_proposal) ** 2, axis=1)
                ...
                for proposal_index, proposal in enumerate(batch_proposals[: exp_objs.shape[1]]):
                    ...
                    if len(new_samples) > 0:
                        min_distance = np.amin(
                            [np.abs(proposal - x) for x in new_samples], axis=0)
