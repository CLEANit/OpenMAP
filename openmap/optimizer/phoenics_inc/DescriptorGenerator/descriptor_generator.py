...
class DescriptorGenerator(Logger):
    ...
    def __init__(self, config):
        ...
        self.exec_name = '%s/DescriptorGenerator/generation_process.py' % self.config.get(
            'home')
    ...
    def single_generate(self, descs, objs, feature_index, result_dict=None):
        ...
        config_name = '%s/descriptor_generation_%d_%s.pkl' % (
            self.config.get('scratch_dir'), feature_index, identifier)
        ...
        results_name = '%s/completed_descriptor_generation_%d_%s.pkl' % (
            self.config.get('scratch_dir'),
            feature_index,
            identifier,
        )
    ...
    def generate(self, obs_params, obs_objs):
        ...
        for feature_index, feature_options in enumerate(self.config.feature_options):
            ...
            self.single_generate(sampled_descriptors,
                                 sampled_objs, feature_index, result_dict)

            # avoid parallel execution if not desired
            if not self.config.get('parallel'):
                if feature_types[feature_index] == 'continuous':
                    continue
                while feature_index not in result_dict:
                    time.sleep(0.1)
        ...
        gen_feature_descriptors = [result_dict[feature_index]
    ...
    def get_summary(self):
        ...
        if not hasattr(self, 'gen_feature_descriptors'):
            for feature_index in range(len(self.config.feature_options)):
                ...
                for desc_index in range(feature_descriptors.shape[1]):
                    ...
                    contribs['descriptor_%d' %
                             desc_index] = copy.deepcopy(desc_summary_dict)
        ...
        for feature_index in range(len(self.config.feature_options)):
            ...
            for index, weight_elements in enumerate(weights):
                normed_weights[index] = weight_elements / \
                    np.sum(np.abs(weight_elements))
            ...
            for new_desc_index in sufficient_indices:
                ...
                cumulative_sum = np.cumsum(
                    np.abs(relevant_weights[sorting_indices]))
                ...
                contribs['descriptor_%d' %
                         new_desc_index] = copy.deepcopy(desc_summary_dict)
            summary['feature_%d' % feature_index] = copy.deepcopy(contribs)
