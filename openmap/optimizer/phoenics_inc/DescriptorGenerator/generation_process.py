...
class Generator(object):
    ...
    def construct_comp_graph(self):
        ...
        self.tf_descs = tf.compat.v1.placeholder(
            tf.float32, [None, self.num_descs])
        ...
        with tf.name_scope('auto_desc_gen'):
            ...
            self.weights_0 = tf.compat.v1.get_variable(
                'weights_0', [self.num_descs, self.num_descs], initializer=tf.initializers.identity()
            )
            self.biases_0 = tf.compat.v1.get_variable(
                'biases_0', [self.num_descs], initializer=tf.initializers.zeros())

            self.weights_0 = self.weights_0 + \
                tf.random.normal([self.num_descs, self.num_descs], 0.0, 1e-5)
            self.biases_0 = self.biases_0 + \
                tf.random.normal([self.num_descs], 0.0, 1e-5)

            def activation(x):
                return tf.nn.softsign(x)

            def regressor(x):
                return activation(tf.matmul(x, self.weights_0) + self.biases_0)

            gen_descs = regressor(self.tf_descs)
            self.gen_descs = gen_descs

            # compute correlation coefficients between descriptors and
            # objectives
            ...
            numerator = tf.reduce_mean(
                (self.tf_objs - objs_mean) * (gen_descs - gen_descs_mean), axis=0)
            ...
            gen_descs_transpose = tf.transpose(
                gen_descs_expand, perm=[0, 2, 1])

            gen_descs_var_expand = tf.expand_dims(gen_descs_var, -1)
            gen_descs_var_transpose = tf.transpose(
                gen_descs_var_expand, perm=[1, 0])

            cov_gen_descs = tf.reduce_mean(
                tf.matmul(gen_descs_expand, gen_descs_transpose), axis=0)
            cov_gen_descs /= tf.sqrt(tf.matmul(gen_descs_var_expand,
                                               gen_descs_var_transpose))
            self.cov_gen_descs = cov_gen_descs

            # compute loss for deviating from target binary matrix
            # min_corr         = 2. * 2. / (3. * np.sqrt(self.num_samples - 1)
            # )    # corresponds to 95 % confidence interval
            min_corr = 1.0 / np.sqrt(self.num_samples - 2)
            self.min_corr = min_corr
            norm_corr_coeffs = tf.nn.leaky_relu(
                (tf.abs(corr_coeffs) - min_corr) / (1.0 - min_corr), 0.01)

            loss_0 = tf.reduce_mean(
                tf.square(tf.sin(np.pi * norm_corr_coeffs)))
            loss_1 = 1.0 - tf.reduce_max(tf.abs(norm_corr_coeffs))
            ...
            norm_cov_x = tf.nn.leaky_relu(
                (tf.abs(cov_gen_descs) - min_corr) / (1.0 - min_corr), 0.01)
            loss_2 = tf.reduce_sum(tf.square(
                tf.sin(np.pi * norm_cov_x / 2.0))) / (self.num_descs ** 2 - self.num_descs)
        ...
        for epoch in range(self.max_iter):
            current_loss, loss_0, loss_1, loss_2, loss_3, gen_descs, _ = self.sess.run(
                [self.loss, self.loss_0, self.loss_1, self.loss_2,
                    self.loss_3, self.weights_0, self.train_op],
                feed_dict={self.tf_descs: self.descs, self.tf_objs: self.objs},
            )

        corr_coeffs, gen_descs_cov = self.sess.run(
            [self.corr_coeffs, self.cov_gen_descs], feed_dict={
                self.tf_descs: self.descs, self.tf_objs: self.objs}
        )
