...
from DatabaseInterfaces import AddEntry, FetchEntries, UpdateEntries
...
class SQLiteDatabase(Logger):
    ...
    def __init__(self, path, attributes, name='table', verbosity=0):
        ...
        self.log('creating database %s at %s' %
                 (self.name, self.db_path), 'DEBUG')
        ...
        for name, att_type in self.attributes.items():
            self.table.append_column(sql.Column(
                name, self.SQLITE_COLUMNS[att_type]))
    ...
    def _return_dict(function):
        def wrapper(self, *args, **kwargs):
            ...
            info_dicts = [{key: entry[key]
                           for key in self.attributes} for entry in entries]
            return info_dicts
    ...
    def _process_requests(self):
        ...
        while keep_processing:
            ...
            keep_processing = (
                len(self.WRITING_REQUESTS) > 0 or len(
                    self.UPDATE_REQUESTS) > 0 or len(self.READING_REQUESTS) > 0
            )
    ...
    def fetch_all(self, condition_dict):
        ...
        for index, key in enumerate(condition_keys):
            if isinstance(condition_values[index], list):
                ...
                if len(condition_values[index]) == 0:
                    return []
                filters = [getattr(self.table.c, key) ==
                           value for value in condition_values[index]]
                condition = sql.or_(*filters)
            else:
                condition = getattr(
                    self.table.c, key) == condition_values[index]
        ...
        self.log('fetched all information from database %s' %
                 self.name, 'DEBUG')
        return entries
    ...
    def update_all(self, condition_dict, update_dict):
        ...
        update = (
            ...
            .values(update_dict)
            .where(getattr(self.table.c, condition_keys[0]) == condition_values[0])
        )
        for index, key in enumerate(condition_keys[1:]):
            update = update.where(getattr(self.table.c, key)
                                  == condition_values[index + 1])
