...
from utilities import Logger, PhoenicsUnknownSettingsError, PhoenicsValueError
...
class DatabaseHandler(DB_Werkzeug, Logger):
    ...
    def __init__(self, config):
        self.config = config
        DB_Werkzeug.__init__(self, self.config, self.DB_ATTRIBUTES,
    ...
    def read_db(self, outfile, verbose):
        ...
        if self.config.get_db('log_runtimes'):
            start_times = [datetime.strptime(
                entry['start_time'], '%Y-%m-%d %H:%M:%S.%f') for entry in db_content]
                    ...
                    for sugg_index in range(len_batch):
                        for param_name in param_names:
                            if param_name not in entry[key][sugg_index]:
                                PhoenicsValueError(
                                    'could not find parameter "%s" in db entry' % param_name)
