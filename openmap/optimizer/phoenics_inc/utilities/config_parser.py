...
class ConfigParser(Logger):
    ...
    def _parse_parameters(self, provided_settings):
        ...
        param_configs = {'name': [], 'type': [], 'size': [],
                         'specifics': [], 'process_constrained': []}
        feature_configs = {'name': [], 'type': [], 'size': [],
                           'specifics': [], 'process_constrained': []}
        kernel_configs = {'name': [], 'type': [], 'size': [],
                          'specifics': [], 'process_constrained': []}
        ...
        for setting in provided_settings:
            ...
            if 'process_constrained' in setting:
                setting['process_constrained'] = bool(
                    setting['process_constrained'])
            else:
            ...
            if setting['type'] == 'continuous':
                ...
                setting['options'] = np.arange(
                    setting['low'], setting['high'], dtype=np.int32)
                setting['descriptors'] = np.arange(
                    setting['low'], setting['high'], dtype=np.float64)
                ...
                if 'category_details' in setting:
                    ...
                    if 'descriptors' not in setting:
                        setting['descriptors'] = None
                        if self.general.auto_desc_gen:
                            self.log(
                                'Automatic descriptor generation requested, but no descriptors provided for parameter "%s". Will not generate descriptors for this parameter. This messag will only be shown once.'
                                % setting['name'],
                                'WARNING',
                            )
                num_cats = len(setting['options'])
            ...
            for key in param_configs.keys():
                if key == 'specifics':
                    element = {spec_key: setting[spec_key]
                ...
                kernel_configs[key].extend(
                    [element for _ in range(size * num_cats)])
    ...
    def _parse_objectives(self, provided_settings):
        ...
        for hier_index in range(len(sort_indices)):
            if np.abs(hier_index - obj_configs['hierarchy'][sort_indices[hier_index]]) > 1e-4:
                self.log(
                    'incorrect objective hierarchy\n\t... please sort objective hierarchies in ascending order, starting from 0',
                    'FATAL',
                )

        # write configuration
        for key in obj_configs:
            self.objectives.add_attr(key, np.array(
                obj_configs[key])[sort_indices])
    ...
    def feature_sizes(self):
        ...
        for feature_index, feature_type in enumerate(self.feature_types):
            if feature_type == 'categorical':
                feature_size = len(
                    self.features.specifics[feature_index]['options'])
    ...
    def kernel_sizes(self):
        ...
        for kernel_index, kernel_type in enumerate(self.kernel_types):
            if kernel_type == 'categorical':
                kernel_size = len(
                    self.kernels.specifics[kernel_index]['options'])
    ...
    def parse(self):
        ...
        if self.config_dict is not None and self.config_file is not None:
            ...
            self.log(
                'Cannot parse configuration due to missing configuration file or configuration dictionary', 'ERROR'
            )
