...
import numpy as np
from utilities import ParserJSON, ParserPickle
...
class CategoryParser(object):
    ...
    def parse(self, file_name=None):
        ...
        try:
            descriptors = np.array([cat_dict['descriptors']
                                    for cat_dict in cat_details])

            # TODO: rescale the descriptors!!
            min_descriptors, max_descriptors = np.amin(
                descriptors, axis=0), np.amax(descriptors, axis=0)
