...
class Acquisition(Logger):
    ...
    def _propose_randomly(self, best_params, num_samples, dominant_samples=None):
        ...
        if dominant_samples is None:
            uniform_samples = self.random_sampler.draw(
                num=self.total_num_vars * num_samples)
            perturb_samples = self.random_sampler.perturb(
                best_params, num=self.total_num_vars * num_samples)
            ...
            for batch_sample in dominant_samples:
                uniform_samples = self.random_sampler.draw(
                    num=self.total_num_vars *
                    num_samples // len(dominant_samples)
                )
                perturb_samples = self.random_sampler.perturb(
                    best_params, num=self.total_num_vars * num_samples)
                samples = np.concatenate([uniform_samples, perturb_samples])
    ...
    def _proposal_optimization_thread(
        self, proposals, kernel_contribution, batch_index, return_index, return_dict=None, dominant_samples=None
    ):
        ...
        if dominant_samples is not None:
            ...
            ignore = np.array([False for _ in range(
                len(self.config.feature_process_constrained))])
    ...
    def _optimize_proposals(self, random_proposals, kernel_contribution, dominant_samples=None):
        ...
        if self.config.get('parallel'):
            ...
            for batch_index in range(len(self.sampling_param_values)):
                for split_index in range(num_splits):
                    ...
                    process = Process(
                        ...
                        args=(
                            random_proposals[split_start:split_end],
                            kernel_contribution,
                            batch_index,
                            return_index,
                            result_dict,
            ...
            for batch_index in range(len(self.sampling_param_values)):
                ...
                result_dict[batch_index] = self._proposal_optimization_thread(
                    random_proposals, kernel_contribution, batch_index, return_index, dominant_samples=dominant_samples
                )
    ...
    def propose(
        ...
        self.local_optimizers = [ParameterOptimizer(
            self.config) for _ in range(len(sampling_param_values))]
