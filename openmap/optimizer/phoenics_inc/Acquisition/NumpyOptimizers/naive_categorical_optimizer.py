...
class NaiveCategoricalOptimizer(AbstractOptimizer):
    ...
    def get_update(self, vector):
        ...
        for pos_index, pos in enumerate(self.pos):
            ...
            # cache the response!
            # 			if self.only_cats:
            # 				func_cand = self.get_response(vector)
            # 			else:
    ...
    for _ in range(10 ** 3):
        ...
        for x_line in lines:
            plt.plot([lines[0], lines[-1]],
                     [x_line, x_line], color='k', lw=0.5)
            plt.plot([x_line, x_line], [
                     lines[0], lines[-1]], color='k', lw=0.5)
        ...
        for x_element in np.arange(highest + 1):
            for y_element in np.arange(highest + 1):
                ...
                plt.plot(x_element, y_element,
                         color=colors[color_index], marker='s', markersize=10, alpha=0.5)
