...
class AdamOptimizer(AbstractOptimizer):
    ...
    def __init__(self, func=None, eta=0.1, beta_1=0.9, beta_2=0.999, epsilon=1e-8, decay=0.0, *args, **kwargs):
    ...
    def get_update(self, vector):
        ...
        eta_next = eta * (np.sqrt(1.0 - np.power(self.beta_2, next_iter)
                                  ) / (1.0 - np.power(self.beta_1, next_iter)))
        ...
        for index, element, grad, mass, vel in zip(range(len(vector)), vector, grads, self.ms, self.vs):
            ...
            v_next = (self.beta_2 * vel) + \
                (1.0 - self.beta_2) * np.square(grad)
            p_next = element - eta_next * m_next / \
                (np.sqrt(v_next) + self.epsilon)
