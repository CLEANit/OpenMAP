...
class ParameterOptimizer(Logger):
    def __init__(self, config):
        self.config = config
        Logger.__init__(self, 'ParamOptimizer',
                        verbosity=self.config.get('verbosity'))

        # parse positions
        self.pos_continuous = np.full(
            self.config.num_features, False, dtype=bool)
        self.pos_categories = np.full(
            self.config.num_features, False, dtype=bool)
        self.pos_discrete = np.full(
            self.config.num_features, False, dtype=bool)
        ...
        if disc_opt_name == 'naive':
            from Acquisition.NumpyOptimizers import NaiveDiscreteOptimizer
        ...
        if cat_opt_name == 'naive':
            from Acquisition.NumpyOptimizers import NaiveCategoricalOptimizer
    ...
    def set_func(self, kernel, ignores=None):
        ...
        self.opt_con.set_func(kernel, pos=np.arange(
            self.config.num_features)[pos_continuous])
        ...
        self.opt_cat.set_func(
            kernel,
            pos=np.arange(self.config.num_features)[pos_categories],
            highest=self.config.feature_sizes[self.pos_categories],
        )
    ...
    def optimize(self, kernel, sample, max_iter=10):
        ...
        if not self.within_bounds(sample):
            sample = np.where(sample < self.config.feature_lowers,
                              self.config.feature_lowers, sample)
            sample = np.where(sample > self.config.feature_uppers,
                              self.config.feature_uppers, sample)
