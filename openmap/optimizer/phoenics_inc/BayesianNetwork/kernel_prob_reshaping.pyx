...
cdef class KernelReshaper:
            ...
            for obs_index in range(self.num_obs):
                ...
                for desc_index in range(self.num_descriptors):
                    ...
                    for kernel_index in range(self.num_kernels):
                        averaged_descriptor = cat_probs[sample_index,
                                                        obs_index,
                                                        kernel_index] * descriptors[kernel_index,
                                                                                    desc_index] + averaged_descriptor
                ...
                for target_cat_index in range(self.num_kernels):
                    ...
                    for desc_index in range(self.num_descriptors):
                        ...
                        dyi = self.num_kernels * \
                            (descriptors[target_cat_index, desc_index] - averaged_descriptor)
                        ds2 = ds2 + dyi * dyi

                    all_distances[sample_index, obs_index, target_cat_index] = sqrt(
                        ds2 / self.num_descriptors)
                ...
                for kernel_index in range(self.num_kernels):
                    recomputed_probs[sample_index,
                                     obs_index,
                                     kernel_index] = exp(- all_distances[sample_index,
                                                                         obs_index,
                                                                         kernel_index]) / sum_distances
    ...
    cpdef reshape_probs(self, np.ndarray cat_probs, np.ndarray descriptors):
        ...
        self.np_recomputed_probs = np.zeros(
            (self.num_samples, self.num_obs, self.num_kernels))
        self.np_all_distances = np.zeros(
            (self.num_samples, self.num_obs, self.num_kernels))

        cdef double[:, :, :] cat_probs_memview = cat_probs
        cdef double[:, :]    descriptors_memview = descriptors

        reshaped_probs = self._reshape_probs(
            cat_probs_memview, descriptors_memview)
