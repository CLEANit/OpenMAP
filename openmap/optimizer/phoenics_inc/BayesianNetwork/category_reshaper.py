...
class CategoryReshaper(Logger):
    def __init__(self, config):
        self.config = config
        Logger.__init__(self, 'CategoryReshaper',
    ...
    def cython_recompute_probs(self, cat_probs, descriptors, index, return_dict=None):
        recomputed_probs = self.kernel_reshaper.reshape_probs(
            cat_probs, descriptors)
                ...
                for target_cat_index in range(num_kernels):
                    ...
                    for desc_index in range(num_descriptors):
                        ...
                        for kernel_index in range(num_kernels):
                            averaged_descriptor += probs[kernel_index] * \
                                descriptors[kernel_index, desc_index]

                        dyi = 0.0
                        for kernel_index in range(num_kernels):
                            partial_contrib = descriptors[target_cat_index,
                                                          desc_index] - averaged_descriptor
                            dyi += partial_contrib

                        ds2 += dyi * dyi

                    distances[target_cat_index] = np.sqrt(
                        ds2 / num_descriptors)

                # got the distances
                rescaled_probs = np.exp(-distances) / \
                    np.sum(np.exp(-distances))
        ...
        if self.config.get('parallel'):
            ...
            for feature_index in range(len(parsed_probs)):
                ...
                process = Process(
                    ...
                    args=(prob_dict['probs'], prob_dict['descriptors'],
                          feature_index, result_dict),
                )
