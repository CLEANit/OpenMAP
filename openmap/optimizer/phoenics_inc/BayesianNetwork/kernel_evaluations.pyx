...
from libc.math cimport abs, exp, round
...
cdef double _gauss(double x, double loc, double sqrt_prec):
    ...
    if argument > 200.:
        ...
        # the number is 1. / np.sqrt(2 * np.pi)
        result = exp(- argument) * sqrt_prec * 0.3989422804014327
    return result
            ...
            for sample_index in range(self.num_samples):
                ...
                while kernel_index < self.num_kernels:
                    ...
                    if kernel_types[kernel_index] == 0:
                        ...
                        prec_prod = prec_prod * \
                            sqrt_precs[sample_index, obs_index, kernel_index]
                        exp_arg_sum = exp_arg_sum + (sqrt_precs[sample_index, obs_index, kernel_index] * (
                            sample[feature_index] - locs[sample_index, obs_index, kernel_index]))**2
                ...
                # * inv_sqrt_two_pi**num_continuous
                ...
                probs[obs_index] = obs_probs * \
                    inv_sqrt_two_pi**num_continuous / self.num_samples
        return probs
        return probs
...
#			for obs_index in range(self.num_obs):
            for sample_index in range(self.num_samples):
                ...
                while kernel_index < self.num_kernels:
                    ...
                    if kernel_types[kernel_index] == 0:
                        total_prob *= _gauss(sample[feature_index],
                                             locs[sample_index,
                                                  obs_index,
                                                  kernel_index],
                                             sqrt_precs[sample_index,
                                                        obs_index,
                                                        kernel_index])
