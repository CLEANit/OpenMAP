...
class NumpyGraph(object):
    def __init__(self, config, model_details):
        self.config = config
        # 		Logger.__init__(self, 'NumpyGraph', self.config.get('verbosity'))
    ...
    def compute_kernels(self, posteriors):
        ...
        for layer_index in range(self._num_layers):
            ...
            for sample_index in range(len(weight)):
                ...
                output = activation(
                    np.matmul(post_layer_outputs[-1], single_weight) + single_bias)
        ...
        # note: np.random.gamma is parametrized with k and theta, while
        # ed.models.Gamma is parametrized with alpha and beta
        post_tau_normed = np.random.gamma(
            self.num_obs +
            np.zeros(post_bnn_output.shape), np.ones(post_bnn_output.shape)
        )
        post_tau = post_tau_normed / tau_rescaling
        post_sqrt_tau = np.sqrt(post_tau)
        post_scale = 1.0 / post_sqrt_tau

        # 		print('BNN OUTPUT', post_bnn_output.shape)
        # 		print('BNN TAU', post_sqrt_tau.shape)
        # 		quit()
        ...
        while kernel_element_index < len(self.config.kernel_names):
            ...
            # 			print('KERNEL_ELEMENT INDEX', kernel_element_index)
            ...
            # 			print('KERNEL', kernel_begin, kernel_end)
            ...
            if kernel_type == 'continuous':
                ...
                post_support = (uppers - lowers) * \
                    (1.2 * sigmoid(post_relevant) - 0.1) + lowers
                ...
                exp_samples = np.exp(
                    (post_support + gumbel_samples) / post_temperature)
