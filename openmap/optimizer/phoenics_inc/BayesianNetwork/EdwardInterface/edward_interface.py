...
class EdwardNetwork(object):
    ...
    def declare_training_data(self, obs_params, obs_objs):
        ...
        # categorical features are encoded as single numbers, bnn output is
        # one-hot encoded
        ...
        for feature_index, feature_type in enumerate(self.config.feature_types):
            ...
            if feature_type == 'categorical':
                for obs_param_index, obs_param in enumerate(obs_params):
                    self.features[obs_param_index, feature_begin +
                                  int(obs_param[feature_index])] += 1
        ...
        self.rescaled_features = (self.features - self.lower_rescalings) / (
            self.upper_rescalings - self.lower_rescalings
        )
        self.rescaled_targets = (self.targets - self.lower_rescalings) / \
            (self.upper_rescalings - self.lower_rescalings)

        # 		self.features = (self.features - lower_rescalings) / (upper_rescalings - lower_rescalings)
        # 		self.targets  = (self.targets  - lower_rescalings) / (upper_rescalings - lower_rescalings)

        # 		print(lower_rescalings)
        # 		print(upper_rescalings)
        # 		print(self.features)
        # 		print(self.rescaled_features)
        # 		quit()
    ...
    def construct_model(self):
        ...
        with self.graph.as_default():
            ...
            # 			self.y = tf.convert_to_tensor(self.rescaled_targets,   dtype = tf.float32)

            self.x = tf.convert_to_tensor(
                self.rescaled_features, dtype=tf.float32)
            ...
            weight_shapes, bias_shapes = [
                [self.feature_size, self.bnn_output_size]], [[self.bnn_output_size]]
            ...
            for layer_index in range(self._num_layers):
                ...
                weight = ed.models.Normal(
                    loc=tf.zeros(weight_shape) + self._weight_loc, scale=tf.zeros(weight_shape) + self._weight_scale
                )
                bias = ed.models.Normal(
                    loc=tf.zeros(bias_shape) + self._bias_loc, scale=tf.zeros(bias_shape) + self._bias_scale
                )
                self.priors['weight_%d' % layer_index] = weight
                self.priors['bias_%d' % layer_index] = bias

                prior_layer_output = activation(
                    tf.matmul(self.prior_layer_outputs[-1], weight) + bias)
                self.prior_layer_outputs.append(prior_layer_output)

            self.prior_bnn_output = self.prior_layer_outputs[-1]
            self.prior_tau_normed = ed.models.Gamma(
                self.num_obs + tf.zeros((self.num_obs, self.bnn_output_size)),
                tf.ones((self.num_obs, self.bnn_output_size)),
            )
            self.prior_tau = self.prior_tau_normed / self.tau_rescaling
            self.prior_scale = ed.models.Deterministic(
                1.0 / tf.sqrt(self.prior_tau))
            ...
            for layer_index in range(self._num_layers):
                ...
                # 				weight = ed.models.Normal(loc = tf.Variable(tf.zeros(weight_shape)), scale = tf.nn.softplus(tf.Variable(tf.zeros(weight_shape))))
                # 				bias   = ed.models.Normal(loc = tf.Variable(tf.zeros(bias_shape)),   scale = tf.nn.softplus(tf.Variable(tf.zeros(bias_shape))))

                weight = ed.models.Normal(
                    loc=tf.Variable(tf.random_normal(weight_shape)),
                    scale=tf.nn.softplus(tf.Variable(tf.zeros(weight_shape))),
                )
                bias = ed.models.Normal(
                    loc=tf.Variable(tf.random_normal(bias_shape)),
                    scale=tf.nn.softplus(tf.Variable(tf.zeros(bias_shape))),
                )

                self.posteriors['weight_%d' % layer_index] = weight
                self.posteriors['bias_%d' % layer_index] = bias

                post_layer_output = activation(
                    tf.matmul(self.post_layer_outputs[-1], weight) + bias)
                self.post_layer_outputs.append(post_layer_output)

            self.post_bnn_output = self.post_layer_outputs[-1]
            self.post_tau_normed = ed.models.Gamma(
                self.num_obs +
                tf.Variable(tf.zeros((self.num_obs, self.bnn_output_size))),
            ...
            while kernel_element_index < len(self.config.kernel_names):
                ...
                prior_relevant = self.prior_bnn_output[:,
                                                       kernel_begin:kernel_end]
                post_relevant = self.post_bnn_output[:,
                                                     kernel_begin:kernel_end]
                ...
                if kernel_type == 'continuous':
                    ...
                    lowers, uppers = (
                        self.config.kernel_lowers[kernel_begin:kernel_end],
                        self.config.kernel_uppers[kernel_begin:kernel_end],
                    )

                    prior_support = (uppers - lowers) * (1.2 *
                                                         tf.nn.sigmoid(prior_relevant) - 0.1) + lowers
                    post_support = (uppers - lowers) * (1.2 *
                                                        tf.nn.sigmoid(post_relevant) - 0.1) + lowers

                    prior_predict = ed.models.Normal(
                        prior_support, self.prior_scale[:, kernel_begin:kernel_end])
                    post_predict = ed.models.Normal(
                        post_support, self.post_scale[:, kernel_begin:kernel_end])
                    ...
                    target = tf.cast(
                        self.y[:, kernel_begin:kernel_end], tf.int32)

                    prior_temperature = 0.5 + 1.0 / self.num_obs
                    ...
                    prior_predict_relaxed = ed.models.RelaxedOneHotCategorical(
                        prior_temperature, prior_support)
                    prior_predict = ed.models.OneHotCategorical(
                        probs=prior_predict_relaxed)

                    post_predict_relaxed = ed.models.RelaxedOneHotCategorical(
                        post_temperature, post_support)
                    post_predict = ed.models.OneHotCategorical(
                    ...
                    PhoenicsUnknownSettingsError(
                        'did not understand parameter type: "%s".\n\tPlease choose from "continuous" or "categorical'
                        % param_type
                    )
            ...
            self.inference.initialize(
                optimizer=self.optimizer, n_iter=self._num_epochs)
    ...
    def sample_thread(self, kernel_value, kernel_name, return_dict):
        ...
        with self.graph.as_default():
            ...
            samples = [kernel_value.sample().eval()
        ...
        for param_index in range(len(self.config.param_names)):
            ...
            if 'loc' in post_kernel and 'sqrt_prec' in post_kernel:
                trace_kernels['locs'].append(
                    post_kernel['loc'].astype(np.float64))
                ...
                trace_kernels['probs'].append(
                    post_kernel['probs'].astype(np.float64))
            else:
                raise NotImplementedError
...
if __name__ == '__main__':
    ...
    edward_network = EdwardNetwork(
        sim_data['config'], sim_data['model_details'])
    edward_network.declare_training_data(
        sim_data['obs_params'], sim_data['obs_objs'])
