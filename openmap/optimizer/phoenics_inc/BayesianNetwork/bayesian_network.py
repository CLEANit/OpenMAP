...
import numpy as np
import pyximport
from BayesianNetwork import CategoryReshaper
...
class BayesianNetwork(Logger):
    ...
    def sample(self, obs_params, obs_objs, num_epochs=None):
        ...
        sim_file = '%s/sampling_information.pkl' % (
            self.config.get('scratch_dir'))
        with open(sim_file, 'wb') as content:
            pickle.dump(sim_data, content)
        results_file = '%s/sampling_results.pkl' % (
            self.config.get('scratch_dir'))

        # submit network sampling
        subprocess.call(
            '%s %s %s %s' % (self.network_executable, self.config.get('home'), sim_file, results_file), shell=True
        )
    ...
    def build_kernels(self, descriptors):
        ...
        self.kernel_evaluator = KernelEvaluator(
            locs, sqrt_precs, probs, kernel_types, kernel_sizes, lower_prob_bound, obs_objs, self.inverse_volume
        )
        ...
        def kernel_contribution(proposed_sample):
            ...
            if self.caching:
                sample_string = '-'.join([str(int(element))
                                          for element in proposed_sample])
                if sample_string in self.cache:
                    num, inv_den = self.cache[sample_string]
                else:
                    num, inv_den, _ = self.kernel_evaluator.get_kernel(
