    ...
    def construct_model(self):
        ...
        with self.graph.as_default():
            ...
            weight_shapes, bias_shapes = [
                [self.feature_size, self._hidden_shape]], [[self._hidden_shape]]
            # 			activations = []
            # 			weight_shapes, bias_shapes = [], []
            ...
            for layer_index in range(self._num_layers):
                ...
                prior_layer_output = activation(
                    tf.matmul(
                        self.prior_layer_outputs[-1], weight.sample()) + bias.sample()
                )
            ...
            self.prior_tau_normed = tfd.Gamma(
                self.num_obs ** 2 +
                tf.zeros((self.num_obs, self.bnn_output_size)),
                tf.ones((self.num_obs, self.bnn_output_size)),
            )
            self.prior_tau = self.prior_tau_normed.sample() / self.tau_rescaling
            self.prior_scale = tfd.Deterministic(1.0 / tf.sqrt(self.prior_tau))
            ...
            for layer_index in range(self._num_layers):
                ...
                weight = tfd.Normal(
                    loc=tf.Variable(tf.random.normal(weight_shape)),
                    scale=tf.nn.softplus(tf.Variable(tf.zeros(weight_shape))),
                )
                bias = tfd.Normal(
                    loc=tf.Variable(tf.random.normal(bias_shape)),
                ...
                post_layer_output = activation(
                    tf.matmul(self.post_layer_outputs[-1], weight.sample()) + bias.sample())
            ...
            self.post_scale = tfd.Deterministic(1.0 / self.post_sqrt_tau)
            ...
            while kernel_element_index < len(self.config.kernel_names):
                ...
                if kernel_type == 'continuous':
                    ...
                    prior_predict = tfd.Normal(
                        prior_support, self.prior_scale[:, kernel_begin:kernel_end].sample())
                    post_predict = tfd.Normal(
                        post_support, self.post_scale[:, kernel_begin:kernel_end].sample())
                    ...
                    prior_predict = tfd.OneHotCategorical(
                        probs=prior_predict_relaxed.sample())

                    post_predict_relaxed = tfd.RelaxedOneHotCategorical(
                        post_temperature, post_support)
                    post_predict = tfd.OneHotCategorical(
                        probs=post_predict_relaxed.sample())
            ...
            for inference in inferences:
                loss += - \
                    tf.reduce_sum(inference['pred'].log_prob(
                        inference['target']))
...
if __name__ == '__main__':
    ...
    tfprob_network = TfprobNetwork(
        sim_data['config'], sim_data['model_details'])
    tfprob_network.declare_training_data(
