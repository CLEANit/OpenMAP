...
class Chimera(object):
    ...
    def rescale(self, raw_objs):
        ...
        for index in range(raw_objs.shape[1]):
            min_objs, max_objs = np.amin(
                raw_objs[:, index]), np.amax(raw_objs[:, index])
            if min_objs < max_objs:
                res_abs[index] = (self.absolutes[index] -
                                  min_objs) / (max_objs - min_objs)
                res_objs[:, index] = (
                    raw_objs[:, index] - min_objs) / (max_objs - min_objs)
            else:
    ...
    def shift_objectives(self, objs, res_abs):
        ...
        for obj_index, obj in enumerate(transposed_objs):
            ...
            maxs.append(maximum)
            tolerance = minimum + \
                self.tolerances[obj_index] * (maximum - minimum)
            ...
            if obj_index < len(transposed_objs) - 1:
                shift -= np.amax(transposed_objs[obj_index + 1]
                                 [domain]) - tolerance
            else:
    ...
    def scalarize_objs(self, shifted_objs, abs_tols):
        ...
        for index in range(0, len(shifted_objs) - 1)[::-1]:
            ...
            scalar_obj += self.step(shifted_objs[index] -
                                    abs_tols[index]) * shifted_objs[index]
