...
class ObservationProcessor(Logger):
    def __init__(self, config):
        ...
        Logger.__init__(self, 'ObservationProcessor',
    ...
    def mirror_parameters(self, param_vector):
        ...
        # mirror param
        params = []
        index_dict_keys, index_dict_values = list(
            index_dict.keys()), list(index_dict.values())
        ...
        for obs_dict in obs_dicts:
            ...
            for param_index, param_name in enumerate(param_names):
                ...
                if param_type == 'continuous':
                    ...
                    obs_param = np.array(
                        [param_options[param_index].index(
                            element) for element in obs_dict[param_name]]
                    )
                elif param_type == 'discrete':
                    obs_param = np.array(
                        [list(param_options[param_index]).index(element)
                         for element in obs_dict[param_name]]
                    )
            ...
            # get obj-vector
            obj_vector = np.array([obs_dict[obj_name]
