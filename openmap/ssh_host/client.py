...
class RemoteClient:
    ...
    def dir_upload(self, dir, remote_path=None):
        ...
        try:
            self.scp.put(dir, recursive=True, remote_path=remote_path)
            # upload = dir
    ...
    def bulk_upload(self, files):
        ...
        logger.info(
            f'All the {len(uploads)}  files  uploaded to {self.host} +++')
    ...
    def bulk_download(self, files, dest):
        ...
        logger.info(
            f' {len(download)} downloaded from {os.path.basename(self.local_path)}')
    ...
    def mkdir_p(self, remote_directory):
        ...
        try:
            ...
            logger.info(
                f'Successfully   created  dir [{basename}]   on {self.host} ')
            return True
    ...
    def sbatch(self, job_directory, job_file='vasp_job.sh'):
        try:
            std_out, _ = self.execute_command(
                f'cd {job_directory}; sbatch {job_file}')
    ...
    def monitor_batch(self, jobs, sleeptime = 300):
        ...
        for job in jobs:
            if not self.check_remote_dir(job['remote_path']):
                ...
                continue
            self.download_file(
                job['remote_path'] + '/' + job['output'], local_directory=job['local_path'])
    ...
    def count_pending_user(self, user=None):
        ...
        try:
            std_out, _ = self.execute_command(
                f'squeue -u {user} --state=pending | wc -l')
            return std_out
        except Exception as e:
            logger.error(f'{e}')
