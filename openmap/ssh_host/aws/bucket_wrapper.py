...
def create_bucket(name, region=None):
    ...
    try:
        if region:
            bucket = s3.create_bucket(Bucket=name, CreateBucketConfiguration={
        ...
        logger.info("Created bucket '%s' in region=%s",
                    bucket.name, s3.meta.client.meta.region_name)
    except ClientError as error:
        logger.exception(
            "Couldn't create bucket named '%s' in region=%s.", name, region)
...
def put_cors(bucket_name, cors_rules):
    ...
    s3 = get_s3()
    try:
        s3.Bucket(bucket_name).Cors().put(
            CORSConfiguration={'CORSRules': cors_rules})
        logger.info("Put CORS rules %s for bucket '%s'.",
                    cors_rules, bucket_name)
    except ClientError:
...
def get_cors(bucket_name):
    ...
    try:
        ...
        logger.info("Got CORS rules %s for bucket '%s'.",
                    cors.cors_rules, bucket_name)
    except ClientError:
...
def get_policy(bucket_name):
    ...
    try:
        ...
        logger.info("Got policy %s for bucket '%s'.",
                    policy.policy, bucket_name)
    except ClientError:
...
def delete_policy(bucket_name):
    ...
    try:
        ...
        logger.exception(
            "Couldn't delete policy for bucket '%s'.", bucket_name)
        raise
...
def put_lifecycle_configuration(bucket_name, lifecycle_rules):
    ...
    try:
        ...
        logger.info("Put lifecycle rules %s for bucket '%s'.",
                    lifecycle_rules, bucket_name)
    except ClientError:
        logger.exception(
            "Couldn't put lifecycle rules for bucket '%s'.", bucket_name)
        raise
...
def get_lifecycle_configuration(bucket_name):
    ...
    try:
        ...
        logger.exception(
            "Couldn't get lifecycle configuration for bucket '%s'.", bucket_name)
        raise
    else:
        return config.rules
...
def delete_lifecycle_configuration(bucket_name):
    ...
    try:
        ...
        logger.info(
            "Deleted lifecycle configuration for bucket '%s'.", bucket_name)
    except ClientError:
        logger.exception(
            "Couldn't delete lifecycle configuration for bucket '%s'.", bucket_name)
        raise
...
def generate_presigned_post(bucket_name, object_key, expires_in):
    ...
    try:
        ...
        logger.exception(
            "Couldn't get a presigned POST URL for bucket '%s' " "and object '%s'", bucket_name, object_key
        )
        raise
    return response


def generate_presigned_url(bucket_name, client_method, method_parameters, expires_in):
    """
    ...
    try:
        ...
        logger.exception(
            "Couldn't get a presigned URL for bucket '%s' " "and client method '%s'.", bucket_name, client_method
        )
        raise
    return url
...
def usage_demo():
    ...
    created_buckets = [
        create_bucket(prefix + str(uuid.uuid1()), s3_resource.meta.client.meta.region_name) for _ in range(3)
    ]
    ...
    try:
        ...
        if error.response['Error']['Code'] == 'MalformedPolicy':
            print(
                "Couldn't put the policy because the specified principal user does "
    ...
    put_rules = [
        {
            'ID': str(uuid.uuid1()),
            'Filter': {'And': {'Prefix': 'monsters/', 'Tags': [{'Key': 'type', 'Value': 'zombie'}]}},
            'Status': 'Enabled',
            'Expiration': {'Days': 28},
        }
    ]
    ...
    url = generate_presigned_url(bucket.name, 'list_objects', {
                                 'Bucket': bucket.name}, 10)
    print(
        f'Generated a pre-signed URL that can be used to list the objects in '
