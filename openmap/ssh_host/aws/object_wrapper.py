...
from openmap.remote_host.aws import bucket_wrapper
...
def copy_object(source_bucket, source_object_key, dest_bucket, dest_object_key):
    ...
    try:
        ...
        obj.copy_from(
            CopySource={'Bucket': source_bucket.name, 'Key': source_object_key})
        obj.wait_until_exists()
        logger.info(
...
def delete_object(bucket, object_key):
    ...
    try:
        ...
        logger.exception(
            "Couldn't delete object '%s' from bucket '%s'.", object_key, bucket.name)
        raise
    ...
    try:
        ...
        if 'Errors' in response:
            logger.warning(
                ...
                [f"{del_obj['Key']}: {del_obj['Code']}" for del_obj in response['Errors']],
                bucket.name,
            )
    except ClientError:
        logger.exception()
...
def usage_demo():
    ...
    bucket = bucket_wrapper.create_bucket(
        'usage-demo-object-wrapper-' +
        str(uuid.uuid1()), bucket_wrapper.s3_resource.meta.client.meta.region_name
    )
