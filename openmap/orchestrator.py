...
from openmap.remote_host.aws import sql_db
from configuration import ChemOs_CONFIG, DataBase_CONFIG, Query
from openmap.remote_host.hpc.resources import projects
...
parser.add_argument('--seed', metavar='N', action='store',
                    dest='seed', type=int, help='seed value')
...
try:
    ...
    print(
        f' The project {args.project} does not exist would you like to create')
...
#     df = data_df.copy()
#     ef = ElementFraction()
#     df.insert(2, 'composition', df.apply(
...
#     measurements = [aws.get_value(
#         DataBase_CONFIG['tablename'], objective_name, id_colm, idx) for idx in sample_id_list]
#     computing_idx = [i for i in range(
#         len(measurements)) if measurements[i] is None]
#
...
#         structures = [mpr.get_structure_by_material_id(
#             idx) for idx in computing_list]
#
#         # 4 generate input
#
#         job_paths = inputgenerator.vasp_input_from_structure(
#             computing_list, structures, objective_name)
#
...
#             campaign_name=campaign_name, local_path=workdir, remote_path=host['sub_text'])
#
